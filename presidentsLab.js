...
//* presidents is an ARRAY, so to get through it we can use and INDEX
//[i]

// const presidents1900 = [];
...
let presidents1900filter = presidents.filter((camel) => camel.year >= 1900);
console.log(presidents1900filter);

// simple filter for this jam

presidents1900filter.forEach((prez) =>
  console.log(prez.first + " " + prez.last)
);

for (prez of presidents1900filter) {
  console.log(prez.first, prez.last);
}
